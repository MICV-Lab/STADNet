...
def compute_metrics(lq_image, hq_image, metrics1=None):
    psnr = metrics.peak_signal_noise_ratio(lq_image, hq_image,data_range=hq_image.max())
    ssim = metrics.structural_similarity(lq_image, hq_image, data_range=hq_image.max())

    if metrics1 is None:
        return {'psnr': [psnr], 'ssim': [ssim]}
    else:
        metrics1['psnr'].append(psnr)
        metrics1['ssim'].append(ssim)
        return metrics
