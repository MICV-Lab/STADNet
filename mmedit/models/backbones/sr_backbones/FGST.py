...
class FGST(nn.Module):
    def __init__(self, dim=32, spynet_pretrained=None, cpu_cache_length=30):
        ...
        main.append(nn.Conv2d(self.dim, self.dim, 3, 1, 1, bias=True))
        # main.append(make_layer(ResidualBlockNoBN, 5, mid_channels=dim))
        # main.append(nn.Conv2d(self.dim, self.dim * 4, 3, 1, 1, bias=True))
        # main.append(nn.LeakyReLU(negative_slope=0.1, inplace=True))
        ...
        self.upconv2 = nn.Conv2d(self.dim, 64 * 4, 3, 1, 1)
    ...
    def compute_flow(self, lqs, flows):
        ...
        flows_results['forward_ds2'] = torch.stack(flows['forward_ds2']).squeeze().cpu().detach().numpy()
        flows_results['backward_ds2'] = torch.stack(flows['backward_ds2']).squeeze().cpu().detach().numpy()
        flows_results['forward_ds4'] = torch.stack(flows['forward_ds4']).squeeze().cpu().detach().numpy()
        flows_results['backward_ds4'] = torch.stack(flows['backward_ds4']).squeeze().cpu().detach().numpy()
    ...
    def forward(self, x):
        ...
        # # print(outputs.shape)
        # sio.savemat('print_out.mat', {'recon': print_out})
