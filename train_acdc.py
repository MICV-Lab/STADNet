...
# model name
parser.add_argument('--experiment_name', type=str, default='acdc_sr_4x', help='give a experiment name before training')
...
# dataset
parser.add_argument('--data_root', type=str, default='./acdc_data', help='data root folder')
parser.add_argument('--list_dir', type=str, default='./acdc_data_list', help='data list_dir root folder')
...
parser.add_argument('--batch_size', type=int, default=2, help='training batch size') # 8
...
parser.add_argument('--step_size', type=int, default=100, help='step size for step scheduler')
...
val_loader = DataLoader(dataset=val_set, num_workers=opts.num_workers, batch_size=2, shuffle=False)
test_loader = DataLoader(dataset=test_set, num_workers=opts.num_workers, batch_size=2, shuffle=False)
...
for epoch in range(ep0, opts.n_epochs + 1):
    ...
    if (epoch+1) % opts.eval_epochs == 0:
        ...
        #     vis_gt = (model.HR.detach()[:, 0:1, :, :] ** 2 + model.HR.detach()[:, 1:2, :, :] ** 2).sqrt()
        #     save_image(vis_gt, gt, normalize=True, scale_each=True, padding=5)
        #     vis_input = (model.LR.detach()[:, 0:1, :, :] ** 2 + model.LR.detach()[:, 1:2, :, :] ** 2).sqrt()
    ...
    if (epoch+1) % opts.save_epochs == 0:
        sio.savemat(os.path.join(image_directory, 'eval_{:03d}.mat'.format(epoch)), model.results)
